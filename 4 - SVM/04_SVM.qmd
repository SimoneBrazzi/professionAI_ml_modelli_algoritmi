...
# Exercise: Heart Attack prediction

```{python}
df = pd.read_csv("~/R/ProfAI_ML_models_algorithms/datasets/heart.csv")
df = df.rename(columns={"output" : "target"})
```

The model is also available at this kaggle [link](https://www.kaggle.com/datasets/rashikrahmanpritom/heart-attack-analysis-prediction-dataset).
...
preprocessor = ColumnTransformer(
    [
        ("std", StandardScaler(), [0, 1] + list(range(3, 13))),
        ("ohe", OneHotEncoder(), [2])
    ],
...
print_classification_report(lr, (xtrain, ytrain))
```

Performance is not so bad, but let think about what does the metrics imply. If a patient risk an heart attack, but it is diagnosted as not at risk, it could die. How many positive data are labeled as positive? This is the recall.
...
As expected, the linear kernel SVC has a very similar result as the base linear regression model.
...
Considering how high dimensional feature space is, it is to be expected the performance is increasing with the *rbf* model.

```{python}
svc = SVC(kernel="rbf", class_weight={1 : 1.5, 0 : .5})
svc.fit(xtrain, ytrain)
print_classification_report(svc, (xtest, ytest))
```
